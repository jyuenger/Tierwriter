...
I'm normally a UK ISO user. While the Enter key here can't span two rows, there are alpha keys for hash, at, grave/negation, and non-US backslash (on row 1), as well as an ISO left Shift. For other langauges that use ISO layout, the Caps Lock key can be remapped as an additional alpha.
